...
class DefaultDepositUseCaseTest {
    ...
    fun `should successfully deposit amount to wallet`() = runTest {
        ...
        result.fold(
            ...
            ifRight = { resultWallet ->
                assertEquals(1500, resultWallet.balance)
    ...
    fun `should return InvalidUserId when userId is blank`() = runTest {
        ...
        result.fold(
            ifLeft = { error ->
                assertTrue(error is DepositError.InvalidUserId)
            },
            ifRight = { }
        )
    ...
    fun `should return NonPositiveAmount when amount is zero`() = runTest {
        ...
        result.fold(
            ifLeft = { error ->
                assertTrue(error is DepositError.NonPositiveAmount)
            },
            ifRight = { }
        )
    ...
    fun `should return WalletNotFound when wallet does not exist`() = runTest {
        ...
        result.fold(
            ifLeft = { error ->
                assertTrue(error is DepositError.WalletNotFound)
            },
            ifRight = { }
        )
    ...
    fun `should return WalletUpdateFailed when wallet update fails`() = runTest {
        ...
        result.fold(
            ifLeft = { error ->
                assertTrue(error is DepositError.WalletUpdateFailed)
            },
            ifRight = { }
        )
    ...
    fun `should return TransactionCreationFailed when transaction creation fails`() = runTest {
        ...
        result.fold(
            ifLeft = { error ->
                assertTrue(error is DepositError.TransactionCreationFailed)
            },
            ifRight = { }
        )
    ...
    fun `should deposit large amount successfully`() = runTest {
        ...
        coEvery { walletRepository.getWalletByUserId(userId) } returns wallet.right()
        coEvery { walletRepository.updateWalletBalance(wallet.id, 1001000) } returns updatedWallet.right()
        coEvery { transactionRepository.create(any()) } returns mockk<org.my.firstcircletest.domain.entities.Transaction>().right()
        ...
        result.fold(
            ifLeft = { },
            ifRight = { resultWallet ->
                assertEquals(1001000, resultWallet.balance)
            }
        )
    }
    ...
    fun `should deposit to wallet with zero balance`() = runTest {
        ...
        coEvery { walletRepository.getWalletByUserId(userId) } returns wallet.right()
        coEvery { walletRepository.updateWalletBalance(wallet.id, 500) } returns updatedWallet.right()
        coEvery { transactionRepository.create(any()) } returns mockk<org.my.firstcircletest.domain.entities.Transaction>().right()
